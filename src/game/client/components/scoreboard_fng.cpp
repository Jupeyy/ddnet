...
void CScoreboard::RenderScoreboardFNG(float x, float y, float w, int Team, const char *pTitle)
{
	...
	int UIRectIndex = 0;
	int Corners = IGraphics::CORNER_BL;
	int CornersTitle = IGraphics::CORNER_TL;
	if(Team == TEAM_BLUE)
	{
		Corners = IGraphics::CORNER_BR;
		CornersTitle = IGraphics::CORNER_TR;
	...
	// background
	RenderTools()->DrawUIElRect(*m_ScoreboardGeneral.Rect(UIRectIndex++), x, y + TitleHeightMax, w, h - TitleHeightMax, gs_BoardColorBG, Corners, ms_TeamScoreboardGroupOffset);
	...
	RenderTools()->DrawUIElRect(*m_ScoreboardGeneral.Rect(UIRectIndex++), x, y, w, TitleHeightMax, ColorTitle, CornersTitle, 10.0f, ERenderToolsQuadTextureMode::RENDER_TOOLS_QUAD_TEXTURE_MODE_RESPECT_REPEATED);
	...
	if(Team == TEAM_RED)
	{
		...
		RenderTools()->DrawUIElRectQuad(*m_ScoreboardGeneral.Rect(UIRectIndex++), x + TitleTextOffset + tw - 25.0f, y, TitleHeightMax, TitleHeightMax, TextureColor, false);
	}
	else if(Team == TEAM_BLUE)
	{
		Graphics()->QuadsSetSubset(0.5f, 0, 1.0f, 1.0f);
		RenderTools()->DrawUIElRectQuad(*m_ScoreboardGeneral.Rect(UIRectIndex++), x + w - TitleTextOffset - tw - 25.0f, y, TitleHeightMax, TitleHeightMax, TextureColor, false);
	}
	Graphics()->QuadsSetSubset(0, 0, 1, 1);
	Graphics()->WrapNormal();

	if(Team == TEAM_RED)
		UI()->DoLabelStreamed(*m_ScoreboardGeneral.Rect(UIRectIndex++), x + TitleTextOffset, y + TitleOffset, w, TitleFontsize, pTitle, TitleFontsize, ETextAlignment::TEXTALIGN_LEFT);
	else if(Team == TEAM_BLUE)
		UI()->DoLabelStreamed(*m_ScoreboardGeneral.Rect(UIRectIndex++), x, y + TitleOffset, w - TitleTextOffset, TitleFontsize, pTitle, TitleFontsize, ETextAlignment::TEXTALIGN_RIGHT);
	else if(Team == TEAM_SPECTATORS)
		UI()->DoLabelStreamed(*m_ScoreboardGeneral.Rect(UIRectIndex++), x, y + TitleOffset, w, TitleFontsize, pTitle, TitleFontsize, ETextAlignment::TEXTALIGN_CENTER);
	...
	if(Team == TEAM_RED)
		UI()->DoLabelStreamed(*m_ScoreboardGeneral.Rect(UIRectIndex++), x, y + ScoreOffset, w - 20.0f, ScoreFontsize, aBuf, ScoreFontsize, ETextAlignment::TEXTALIGN_RIGHT, -1, 0);
	else if(Team == TEAM_BLUE)
		UI()->DoLabelStreamed(*m_ScoreboardGeneral.Rect(UIRectIndex++), x + 20.0f, y + ScoreOffset, w, ScoreFontsize, aBuf, ScoreFontsize, ETextAlignment::TEXTALIGN_LEFT, -1, 0);
	...
	RenderTools()->DrawUIElRect(*m_ScoreboardGeneral.Rect(UIRectIndex++), x, y, w, ScoreboardHeadlineMaxSize, gs_ColorHeader, 0, 0.0f);
	if(Team == TEAM_RED)
		RenderTools()->DrawUIElRect3D(*m_ScoreboardGeneral.Rect(UIRectIndex++), x, y, -OverSize3D, ScoreboardHeadlineMaxSize, gs_ColorHeader, -Radius3D);
	else if(Team == TEAM_BLUE)
		RenderTools()->DrawUIElRect3D(*m_ScoreboardGeneral.Rect(UIRectIndex++), x + w, y, OverSize3D, ScoreboardHeadlineMaxSize, gs_ColorHeader, Radius3D);
	...
	if(Team != TEAM_SPECTATORS)
		UI()->DoLabelStreamed(*m_ScoreboardGeneral.Rect(UIRectIndex++), TeeScoreOffset, y + ScoreboardHeadlineTextOffset, ScoreLength, ScoreboardHeadlineFontsize, pScore, ScoreboardHeadlineFontsize, ETextAlignment::TEXTALIGN_RIGHT);

	UI()->DoLabelStreamed(*m_ScoreboardGeneral.Rect(UIRectIndex++), NameOffset, y + ScoreboardHeadlineTextOffset, WidthEntries, ScoreboardHeadlineFontsize, Localize("Name"), ScoreboardHeadlineFontsize, ETextAlignment::TEXTALIGN_LEFT);
	UI()->DoLabelStreamed(*m_ScoreboardGeneral.Rect(UIRectIndex++), ClanOffset, y + ScoreboardHeadlineTextOffset, WidthEntries, ScoreboardHeadlineFontsize, Localize("Clan"), ScoreboardHeadlineFontsize, ETextAlignment::TEXTALIGN_LEFT);

	if(Team != TEAM_SPECTATORS)
		UI()->DoLabelStreamed(*m_ScoreboardGeneral.Rect(UIRectIndex++), PingOffset, y + ScoreboardHeadlineTextOffset, WidthEntries, ScoreboardHeadlineFontsize, Localize("Ping"), ScoreboardHeadlineFontsize, ETextAlignment::TEXTALIGN_LEFT);
	...
	for(int i = 0; i < MAX_CLIENTS; i++)
	{
		...
		if(pInfo->m_Local || (m_pClient->m_Snap.m_SpecInfo.m_Active && pInfo->m_ClientID == m_pClient->m_Snap.m_SpecInfo.m_SpectatorID))
		{
			...
			if(Team == TEAM_RED)
				RenderTools()->DrawUIElRect3D(*m_ScoreboardPlayers[i].Rect(PlayerUIRectIndex++), XEntries - SmallSpacing, YEntries, -OverSize3D, LineHeight, ColorInner, -Radius3D, 0.35f);
			else if(Team == TEAM_BLUE)
				RenderTools()->DrawUIElRect3D(*m_ScoreboardPlayers[i].Rect(PlayerUIRectIndex++), XEntries - SmallSpacing + WidthEntries + SmallSpacingSize, YEntries, OverSize3D, LineHeight, ColorInner, Radius3D, 0.35f);
			RenderTools()->DrawUIElRect(*m_ScoreboardPlayers[i].Rect(PlayerUIRectIndex++), XEntries - SmallSpacing, YEntries, WidthEntries + SmallSpacingSize, LineHeight, ColorInner, 0, 0.0f);
		}
		...
		if(Team != TEAM_SPECTATORS)
		{
			...
			UI()->DoLabelStreamed(*m_ScoreboardPlayers[i].Rect(PlayerUIRectIndex++), TeeScoreOffset, YEntries + LineInfoOffset, ScoreLength, FontSize, aBuf, FontSize, ETextAlignment::TEXTALIGN_RIGHT);
		}
		...
		if(g_Config.m_ClShowIDs)
		{
			...
			UI()->DoLabelStreamed(*m_ScoreboardPlayers[i].Rect(PlayerUIRectIndex++), NameOffset, YEntries + LineInfoOffset, WidthEntries, FontSize, aId, FontSize, ETextAlignment::TEXTALIGN_LEFT, NameLength, 1, true);
		}
		else
		{
			UI()->DoLabelStreamed(*m_ScoreboardPlayers[i].Rect(PlayerUIRectIndex++), NameOffset, YEntries + LineInfoOffset, WidthEntries, FontSize, m_pClient->m_aClients[pInfo->m_ClientID].m_aName, FontSize, ETextAlignment::TEXTALIGN_LEFT, NameLength, 1, true);
		}
		...
		if(m_pClient->m_aClients[pInfo->m_ClientID].m_aClan[0] != '\0')
			UI()->DoLabelStreamed(*m_ScoreboardPlayers[i].Rect(PlayerUIRectIndex++), ClanOffset, YEntries + LineInfoOffset, WidthEntries, FontSize, m_pClient->m_aClients[pInfo->m_ClientID].m_aClan, FontSize, ETextAlignment::TEXTALIGN_LEFT, ClanLength, 1, true);
		...
		if(Team != TEAM_SPECTATORS)
		{
			...
			UI()->DoLabelStreamed(*m_ScoreboardPlayers[i].Rect(PlayerUIRectIndex++), PingOffset, YEntries + LineInfoOffset, PingLength, FontSize, aBuf, FontSize, ETextAlignment::TEXTALIGN_RIGHT, PingLength, 1, true);
		}
		...
		RenderTools()->DrawUIElRect(*m_ScoreboardPlayers[i].Rect(PlayerUIRectIndex++), XEntries - SmallSpacing, y, WidthEntries + SmallSpacingSize, Spacing, ColorInner, 0, 0.0f);
		y += Spacing;
	}
	...
	if(Team != TEAM_SPECTATORS)
	{
		RenderTools()->DrawUIElRect(*m_ScoreboardGeneral.Rect(UIRectIndex++), x, y, w, ScoreboardHeadlineMaxSize, gs_ColorFeet, 0, 0.0f);
	}
	...
	if(m_pClient->m_Snap.m_pGameInfoObj)
	{
		...
		if(Team == TEAM_RED)
		{
			if(m_pClient->m_Snap.m_pGameInfoObj->m_ScoreLimit)
			{
				...
				UI()->DoTextLabel(XEntries, y + ScoreboardFootlineTextOffset, WidthEntries, ScoreboardFootlineFontsize, aBuf, ScoreboardFootlineFontsize, ETextAlignment::TEXTALIGN_LEFT);
			}
			if(m_pClient->m_Snap.m_pGameInfoObj->m_TimeLimit)
			{
				...
				UI()->DoTextLabel(XEntries, y + ScoreboardFootlineTextOffset, WidthEntries, ScoreboardFootlineFontsize, aBuf, ScoreboardFootlineFontsize, ETextAlignment::TEXTALIGN_RIGHT);
			}
		}
		else if(Team == TEAM_BLUE)
		{
