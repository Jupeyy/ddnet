	...
	// do seekbar
	{
		...
		if(UI()->ActiveItem() == id)
		{
			...
			else
			{
				...
				float SeekAmount = (UI()->MouseX() - SeekBar.x) / SeekBar.w;

				if(Input()->KeyIsPressed(KEY_LSHIFT) || Input()->KeyIsPressed(KEY_RSHIFT))
				{
					SeekAmount = PrevAmount + (SeekAmount - PrevAmount) * 0.05f;

					if(SeekAmount > 0.0f && SeekAmount < 1.0f && absolute(PrevAmount - SeekAmount) >= 0.0001f)
					{
						m_pClient->m_SuppressEvents = true;
						DemoPlayer()->SeekPercent(SeekAmount);
				...
				{
					if(SeekAmount > 0.0f && SeekAmount < 1.0f && absolute(PrevAmount - SeekAmount) >= 0.001f)
					{
						PrevAmount = SeekAmount;
						m_pClient->m_SuppressEvents = true;
						DemoPlayer()->SeekPercent(SeekAmount);
						m_pClient->m_SuppressEvents = false;
						m_pClient->m_MapLayersBackGround.EnvelopeUpdate();
	...
	*/
	...
	for(sorted_array<CDemoItem>::range r = m_lDemos.all(); !r.empty(); r.pop_front())
	{
		...
		if(Row.y + Row.h > OriginalView.y && Row.y < OriginalView.y + OriginalView.h)
		{
			if(Selected)
			{
				CUIRect HoverRect = Row;
				HoverRect.Margin(0.5f, &HoverRect);
				RenderTools()->DrawUIRect(&HoverRect, ColorRGBA(1, 1, 1, 0.5f), CUI::CORNER_ALL, 4.0f);
			}
			else if(UI()->MouseInside(&SelectHitBox))
			{
				CUIRect HoverRect = Row;
				HoverRect.Margin(0.5f, &HoverRect);
				RenderTools()->DrawUIRect(&HoverRect, ColorRGBA(1, 1, 1, 0.25f), CUI::CORNER_ALL, 4.0f);
			}
