...
protected:
	...
	************************/
	...
	void RegisterCommands()
	{
		...
		m_aCommandCallbacks[CommandBufferCMDOff(CCommandBuffer::CMD_CLEAR)] = {false, [](SRenderCommandExecuteBuffer &ExecBuffer, const CCommandBuffer::SCommand *pBaseCommand) {}, [this](const CCommandBuffer::SCommand *pBaseCommand, SRenderCommandExecuteBuffer &ExecBuffer) { Cmd_Clear(static_cast<const CCommandBuffer::SCommand_Clear *>(pBaseCommand)); return true; }};
	...
	************************/
	...
	bool GetPresentedImageDataImpl(uint32_t &Width, uint32_t &Height, uint32_t &Format, std::vector<uint8_t> &DstData, bool FlipImgData, bool ResetAlpha)
	{
		...
		{
			Width = m_VKSwapImgExtent.width;
			Height = m_VKSwapImgExtent.height;
			...
			Region.imageExtent = {m_VKSwapImgExtent.width, m_VKSwapImgExtent.height, 1};
	...
	{
		return GetPresentedImageDataImpl(Width, Height, Format, DstData, true, false);
	}
		...
		if(m_ThreadCount > 1)
		{
			...
			for(size_t ThreadIndex = 0; ThreadIndex < m_ThreadCount - 1; ++ThreadIndex)
			{
				if(m_ThreadHelperHadCommands[ThreadIndex])
				{
					...
					while(pRenderThread->m_IsRendering)
					{
						pRenderThread->m_Cond.wait(Lock);
					}
	...
	************************/
	...
	void RenderTileLayer_FillExecuteBuffer(SRenderCommandExecuteBuffer &ExecBuffer, const CCommandBuffer::SState &State, size_t BufferContainerIndex)
	{
	...
	void RenderTileLayer(SRenderCommandExecuteBuffer &ExecBuffer, const CCommandBuffer::SState &State, int Type, const GL_SColorf &Color, const vec2 &Dir, const vec2 &Off, int32_t JumpIndex, size_t IndicesDrawNum, char *const *pIndicesOffsets, const unsigned int *pDrawCount, size_t InstanceCount, size_t FirstInstanceIndex)
	{
		...
		std::array aOffsets = {ExecBuffer.m_BufferOff};
...
public:
	...
	************************/
	...
	void Cmd_Clear(const CCommandBuffer::SCommand_Clear *pCommand)
	{
		...
		{
			std::array aAttachments = {VkClearAttachment{VK_IMAGE_ASPECT_COLOR_BIT, 0, VkClearValue{VkClearColorValue{{m_aClearColor[0], m_aClearColor[1], m_aClearColor[2], m_aClearColor[3]}}}}};
			std::array aClearRects = {VkClearRect{{{0, 0}, m_VKSwapImgExtent}, 0, 1}};
			vkCmdClearAttachments(GetMainGraphicCommandBuffer(), aAttachments.size(), aAttachments.data(), aClearRects.size(), aClearRects.data());
		}
	}
	...
	{
		RenderTileLayer_FillExecuteBuffer(ExecBuffer, pCommand->m_State, pCommand->m_BufferContainerIndex);
	}
	...
	void Cmd_RenderQuadLayer(const CCommandBuffer::SCommand_RenderQuadLayer *pCommand, SRenderCommandExecuteBuffer &ExecBuffer)
	{
		...
		VkDeviceSize IndexOffset = (VkDeviceSize)((ptrdiff_t)(pCommand->m_QuadOffset) * 6 * sizeof(uint32_t));
		...
		const int MaxQuadCount = 256;
		int DrawCount = pCommand->m_QuadNum;
		size_t RenderOffset = 0;

		while(DrawCount > 0)
		{
			int RealDrawCount = (DrawCount > MaxQuadCount ? MaxQuadCount : DrawCount);

			if(!CanBePushed)
			{
				IndexOffset = (VkDeviceSize)((ptrdiff_t)(pCommand->m_QuadOffset + RenderOffset) * 6 * sizeof(uint32_t));
				vkCmdBindIndexBuffer(CommandBuffer, m_RenderIndexBuffer, IndexOffset, VK_INDEX_TYPE_UINT32);
			...
			vkCmdDrawIndexed(CommandBuffer, static_cast<uint32_t>(RealDrawCount * 6), 1, 0, 0, 0);

			RenderOffset += MaxQuadCount;
			DrawCount -= MaxQuadCount;
		}
	}
	...
	{
		BufferContainer_FillExecuteBuffer(ExecBuffer, pCommand->m_State, (size_t)pCommand->m_BufferContainerIndex);
	}
